...
func (s *Service) StartFromSavedState(saved state.BeaconState) error {
	...
	if s.cfg.P2P == nil {
		log.Warning("P2P is not configured, cannot update custody info")
		return nil
	}
...
func (s *Service) updateCustodyInfoInDB() (primitives.Slot, uint64, error) {
	...
	// Retrieve the head slot.
	headBlock, err := s.cfg.BeaconDB.HeadBlock(s.ctx)
	if err != nil {
		return 0, 0, errors.Wrap(err, "head slot")
	}

	if headBlock == nil || headBlock.IsNil() {
		log.Warning("Head block is nil, cannot update custody info")
		return 0, custodyGroupCount, nil
	}

	referenceSlot := headBlock.Block().Slot()

	// If the head slot is before the fulu fork slot, then use the earliest stored slot as the reference slot.
	if referenceSlot < fuluForkSlot {
		referenceSlot, err = s.cfg.BeaconDB.EarliestSlot(s.ctx)
		if err != nil {
			return 0, 0, errors.Wrap(err, "earliest slot")
		}
	}

	custodyGroupCount, earliestAvailableSlot, err := s.cfg.BeaconDB.UpdateCustodyInfo(s.ctx, custodyGroupCount, referenceSlot)
	if err != nil {
